...
impl Scene{
    ...
    pub fn test_scene(&mut self) {
        let sphere1 = Sphere::new(Point::new(2.0, 0.0, 4.0), 1.0, Color::new_from_color(Colors::Red), 0.1, Material::Diffuse); 
        self.elements.push(Element::Sphere(sphere1)); 

        let sphere2 = Sphere::new(Point::new(-1.0, 0.0, 4.0), 1.0, Color::new_from_color(Colors::Green), 0.1, Material::Reflective(0.9)); 
        self.elements.push(Element::Sphere(sphere2));

        let sphere3 = Sphere::new(Point::new(-1.0, -0.5, 1.5), 0.5, Color::new_from_color(Colors::Blue), 0.1, Material::Reflective(0.9)); 
        self.elements.push(Element::Sphere(sphere3));

        let plane1 = Plane::new(Point::new(0.0, -1.0, 0.0), Vector3::new(0.0, -1.0, 0.0), Color::new_from_rgba(55., 57., 67., 0.), 0.1, Material::Diffuse);
        ...
        let sph_light1 = SphericalLight::new(Point::new(2.0, 2.0, 0.0), 2.0, Color::new_from_color(Colors::White), 2000.0); 
    ...
    pub fn normal_shading(&self, ray: &Ray, element: &Element, intersection: Point) -> Color{
        ...
        for light in self.spherical_lights.iter(){
            ...
            let direction_to_light = light.center.subtract(&intersection).to_vector3().normalize(); 
            let shadow_ray = Ray::create_ray(intersection.add(&element.normal(&intersection).multiply(0.00001).to_point()), direction_to_light.copy(), 1); 
            let shadow_element = self.get_closest_element(&shadow_ray);
            let shadow_element_intersect = self.elements[shadow_element as usize].intersect(&shadow_ray);  

            let light_blocked = if shadow_element == -1.0 || (light_intersect.1 > shadow_element_intersect.1) {false} else {true}; 
            ...
            let light_power = (element.normal(&intersection).dot(&direction_to_light)).max(0.0) * light_intensity; 
        ...
        for light in self.directional_lights.iter(){
            ...
            let shadow_ray = Ray::create_ray(intersection.add(&direction_to_light.multiply(0.00001).to_point()), direction_to_light.copy(), 1); 
            let shadow_element = self.get_closest_element(&shadow_ray);
            //println!("Intersection: {},{},{}", intersection.x, intersection.y, intersection.z); 
    ...
    pub fn get_color(&self, ray: &Ray) -> Color{
        ...
        let element_intersection = ray.origin.add(&ray.direction.multiply(element_intersect.1).to_point()); 
        ...
        match self.elements[closest_element as usize].get_material(){
            Material::Diffuse => self.normal_shading(ray, &self.elements[closest_element as usize], element_intersection),
            Material::Reflective(reflectivity) => {
                let mut color = self.normal_shading(ray, &self.elements[closest_element as usize], element_intersection.copy()); 
